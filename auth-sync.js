...
function recoverLocalDraftIfNeeded(remotePayload, remoteUpdatedAt = null) {
  ...
  pendingDraftRecovery = { draft, remotePayload };
  showDraftRecoveryModal(draft, remoteUpdatedAt);
  return true;
}

function showDraftRecoveryModal(draft, remoteUpdatedAt) {
  const sub = document.getElementById('draft-recovery-sub');
  const preview = document.getElementById('draft-recovery-preview');
  ...
  const serverTime = remoteUpdatedAt
    ? new Date(remoteUpdatedAt).toLocaleString('pt-BR', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })
    : null;

  if (sub) sub.textContent = `Edições locais de ${draftTime} não foram sincronizadas com o servidor.`;
  if (preview) {
    preview.innerHTML = `
      <strong>O que aconteceu?</strong>
      <span>O app foi fechado antes de salvar — o rascunho ficou guardado localmente.</span>
      ${serverTime ? `<strong>Versão do servidor</strong><span>Última sincronização: ${serverTime}</span>` : ''}
      <strong>Rascunho local</strong><span>Salvo em: ${draftTime}</span>
    `;
  }

  document.getElementById('draft-recovery-overlay')?.classList.add('open');
}

function confirmDraftRecovery() {
  if (!pendingDraftRecovery) return;
  const { draft, remotePayload } = pendingDraftRecovery;
  pendingDraftRecovery = null;
  document.getElementById('draft-recovery-overlay')?.classList.remove('open');
...
function discardDraftRecovery() {
  if (!pendingDraftRecovery) return;
  const { remotePayload } = pendingDraftRecovery;
  pendingDraftRecovery = null;
  document.getElementById('draft-recovery-overlay')?.classList.remove('open');

  clearLocalDraft();
  if (remotePayload) {
    applySnapshotPayload(remotePayload);
    setSyncBasePayload(remotePayload);
  } else {
    resetStateData();
    initData();
